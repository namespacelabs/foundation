...
func RenderPortsAndIngresses(checkmark bool, out io.Writer, localHostname string, stack *schema.Stack, focus []*schema.Server, portFwds []*PortFwd, ingressDomains []*runtime.FilteredDomain, ingress []*schema.IngressFragment) {
	...
	renderIngress(checkmark, out, cloudDomains, 443, "Internet-facing ingress endpoints")
	...
	if ingressFwdCount == 0 && len(nonLocalIngress) > 0 {
		...
		for _, n := range nonLocalIngress {
			...
			fmt.Fprintf(out, " %s%s%s%s%s%s%s\n", checkLabel(true, true, 0), schema, n.Domain.Fqdn, portLabel, spaces, suffix, comment(n.Owner))
		}
	}
}
