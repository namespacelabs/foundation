...
import { handleServerFunction, handleUntracedServerFunction } from "./serverUtils";
import { getMethodsToWrap, makeGrpcClientRemoteCall, getMetadata } from "./clientUtils";
...
export class GrpcJsInstrumentation extends InstrumentationBase {
		...
		return [
			new InstrumentationNodeModuleDefinition<typeof grpcJs>(
				...
				(moduleExports, version) => {
					...
					this._wrap(moduleExports.Server.prototype, "register", this._patchServer() as any);
	 ...
	 */
	private _patchServer(): (originalRegister: ServerRegisterFunction) => ServerRegisterFunction {
		...
		return (originalRegister: ServerRegisterFunction) => {
			...
			return function register<RequestType, ResponseType>(
				...
				instrumentation._wrap(handlerSet, "func", (originalFunc: HandleCall<unknown, unknown>) => {
						...
						context.with(
							...
							() => {
								...
								context.with(trace.setSpan(context.active(), span), () => {
									handleServerFunction.call(self, span, type, originalFunc, call, callback);
								});
							}
						);
					};
	...
	private _getPatchedClientMethods(
		...
		return (original: GrpcClientFunc) => {
			...
			function clientMethodTrace(this: grpcJs.Client) {
				...
				const metadata = getMetadata.call(instrumentation, grpcClient, original, args);
