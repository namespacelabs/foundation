...
import { _grpcStatusCodeToSpanStatus, _grpcStatusCodeToOpenTelemetryStatusCode } from "./utils";
...
export function makeGrpcClientRemoteCall(
	 ...
	 */
	function patchedCallback<ResponseType>(
		span: Span,
		...
		const wrappedFn: SendUnaryDataCallback<ResponseType> = (
			...
			if (err) {
				if (err.code) {
					...
					span.setAttribute(SemanticAttributes.RPC_GRPC_STATUS_CODE, err.code.toString());
				}
				span.setAttributes({
				...
				span.setAttribute(SemanticAttributes.RPC_GRPC_STATUS_CODE, SpanStatusCode.UNSET.toString());
			}
