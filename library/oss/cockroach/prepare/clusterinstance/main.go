...
func main() {
	...
	henv.HandleApply(func(ctx context.Context, req provisioning.StackRequest, out *provisioning.ApplyOutput) error {
		intent := &cockroach.ClusterIntent{}
		...
		srv := intent.Server
		if srv == nil {
			srv = schema.MakePackageSingleRef("namespacelabs.dev/foundation/library/oss/cockroach/server")
		}

		password := intent.PasswordSecret
		if password == nil {
			password = schema.MakePackageRef("namespacelabs.dev/foundation/library/oss/cockroach/server", "password")
		}
