...
import (
	...
	"namespacelabs.dev/foundation/library/oss/redis"
...
func main() {
	...
	henv.HandleApply(func(ctx context.Context, req provisioning.StackRequest, out *provisioning.ApplyOutput) error {
		...
		srv := intent.Server
		if srv == nil {
			srv = schema.MakePackageSingleRef("namespacelabs.dev/foundation/library/oss/redis/server")
		}

		password := intent.PasswordSecret
		if password == nil {
			password = schema.MakePackageRef("namespacelabs.dev/foundation/library/oss/redis/server", "password")
		}
