...
# Branched from on https://github.com/MariaDB/mariadb-docker/blob/master/10.7/docker-entrypoint.sh
...
_main() {
	...
	if [ "$1" = 'mariadbd' ] || [ "$1" = 'mysqld' ] && ! _mysql_want_help "$@"; then
		...
		if [ -z "$DATABASE_ALREADY_EXISTS" ]; then
			...
			# Namespace: edit begin
			# Skip init files as we perform database initialization through init containers.
            # We can't easily skip the entire temp server, since docker_setup_db grants external connections.
			#
