...
func (r k8sRuntime) RunOneShot(ctx context.Context, pkg schema.PackageName, runOpts runtime.ServerRunOpts, logOutput io.Writer) error {
	...
	pod := applycorev1.Pod(name, r.ns(pkg)).
		WithSpec(applycorev1.PodSpec().WithContainers(container).WithRestartPolicy(corev1.RestartPolicyNever))

	if _, err := cli.CoreV1().Pods(r.ns(pkg)).Apply(ctx, pod, kubedef.Ego()); err != nil {
		return err
	}

	if err := r.Wait(ctx, tasks.Action("kubernetes.pod.deploy").Scope(pkg),
		WaitForPodConditition(fetchPod(r.ns(pkg), name), func(status corev1.PodStatus) (bool, error) {
	...
	if err := fetchPodLogs(ctx, cli, logOutput, r.ns(pkg), name, "", runtime.StreamLogsOpts{Follow: true}); err != nil {
		return err
	}

	for k := 0; ; k++ {
		finalState, err := cli.CoreV1().Pods(r.ns(pkg)).Get(ctx, name, metav1.GetOptions{})
		if err != nil {
		...
		for _, containerStatus := range finalState.Status.ContainerStatuses {
			if term := containerStatus.State.Terminated; term != nil {
				if k > 0 {
					...
					_ = fetchPodLogs(ctxWithTimeout, cli, logOutput, r.ns(pkg), name, "", runtime.StreamLogsOpts{TailLines: 50})
				}
		...
		if err := r.Wait(ctx, tasks.Action("kubernetes.pod.wait"), WaitForPodConditition(fetchPod(r.ns(pkg), name), func(status corev1.PodStatus) (bool, error) {
