...
Buck2 authenticates with a short-lived mTLS client certificate by default.
Pass --static to embed an expiring bearer token instead.`,
		...
		fncobra.DurationVar(flags, &staticDur, "static_token_duration", defaultBuck2TokenDuration, "The minimum duration of the token embedded in the buckconfig (requires --static).")
	}).Do(func(ctx context.Context) error {
		if err := validateBuck2SetupFlags(output, staticDur, instanceName); err != nil {
			return err
		}
		...
		resp, err := retryBazelProvisioning(ctx, func() (*connect.Response[bazelv1beta.EnsureBazelCacheResponse], error) {
			return client.EnsureBazelCache(ctx, connect.NewRequest(makeEnsureBazelCacheRequest(1, !static, false, "")))
		})
		if err != nil {
			return fnerrors.Newf("failed to provision bazel cache: %w", err)
		}

		endpoint := resp.Msg.GetCacheEndpoint()
		if static {
			endpoint = resp.Msg.GetHttpsCacheEndpoint()
		}
		if endpoint == "" {
			return fnerrors.Newf("did not receive a cache endpoint for the selected authentication mode")
		}

		out, err := buck2CacheOnlySetup(endpoint, instanceName)
		if err != nil {
			return err
		}
		...
		} else if resp.Msg.GetUseWorkloadMtls() {
			if err := issueBuck2ClientCertificate(ctx, &out); err != nil {
				return err
			}
		} else {
			if err := setBuck2ClientIdentity(&out, []byte(resp.Msg.GetClientCertPem()), []byte(resp.Msg.GetClientKeyPem())); err != nil {
				return err
			}
			out.serverCAPem = []byte(resp.Msg.GetServerCaPem())
		}

		readiness, cleanup, err := buck2ReadinessConfig(endpoint, out)
		if err != nil {
			return err
		}
		defer cleanup()
		if err := waitForBazelCacheReady(ctx, readiness); err != nil {
			return fnerrors.Newf("failed waiting for Buck2 cache readiness: %w", err)
		}
...
Pass --static to embed an expiring bearer token instead.`,
		...
		} else if err := issueBuck2ClientCertificate(ctx, &out); err != nil {
			return err
		}

		readiness, cleanup, err := buck2ReadinessConfig(storageEndpoint, out)
		if err != nil {
			return err
		}
		defer cleanup()
		if err := waitForBazelCacheReady(ctx, readiness); err != nil {
			return fnerrors.Newf("failed waiting for Buck2 storage readiness: %w", err)
		}
...
func validateBuck2SetupFlags(output string, tokenDuration time.Duration, instanceName string) error {
	...
	if strings.ContainsAny(instanceName, "\r\n") {
		return fnerrors.BadInputError("--instance_name must not contain a newline")
	}
	if match := buck2EnvVarRef.FindString(instanceName); match != "" {
		return fnerrors.BadInputError("--instance_name contains %q, which Buck2 would expand as an environment variable", match)
	}
...
func issueBuck2ClientCertificate(ctx context.Context, out *buck2Setup) error {
	...
	return setBuck2ClientIdentity(out, []byte(clientCertPem), privateKeyPem)
}
...
func buck2ReadinessConfig(endpoint string, out buck2Setup) (bazelCacheReadinessConfig, func(), error) {
	cfg := bazelCacheReadinessConfig{
		endpoint:    endpoint,
		bearerToken: out.StaticToken,
	...
	f, err := os.CreateTemp("", "nsc-buck2-client-*.pem")
	if err != nil {
		return bazelCacheReadinessConfig{}, nil, fnerrors.Newf("failed to create temporary client identity: %w", err)
	}
	cleanup := func() {
		_ = os.Remove(f.Name())
	}
	if _, err := f.Write(out.clientIdentity); err != nil {
		_ = f.Close()
		cleanup()
		return bazelCacheReadinessConfig{}, nil, fnerrors.Newf("failed to write temporary client identity: %w", err)
	}
	if err := f.Close(); err != nil {
		cleanup()
		return bazelCacheReadinessConfig{}, nil, fnerrors.Newf("failed to close temporary client identity: %w", err)
	}

	cfg.clientCert = f.Name()
	cfg.clientKey = f.Name()
...
func toBuck2Config(out buck2Setup) ([]byte, error) {
	...
	if out.TLSCACerts != "" {
		fmt.Fprintf(&buf, "tls_ca_certs = %s\n", out.TLSCACerts)
	}
...
func emitBuck2Config(ctx context.Context, out buck2Setup, buckConfigPath, output, configuration string) error {
	...
	if len(out.serverCAPem) > 0 {
		out.TLSCACerts, err = filepath.Abs(path + ".ca.pem")
		if err != nil {
			return fnerrors.Newf("failed to resolve the server CA path: %w", err)
		}
	}
	...
	if len(out.serverCAPem) > 0 {
		if err := writeCredentialFile(out.TLSCACerts, out.serverCAPem); err != nil {
			return err
		}
	}
...
// writeCredentialFile writes configuration and credentials with restrictive
// permissions because they may contain a bearer token or private key.
