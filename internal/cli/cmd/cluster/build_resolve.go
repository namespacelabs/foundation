...
func newBuildResolveCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Example: `nsc build resolve --github_job <job-id>
nsc build resolve --buildkite_job <job-id>
nsc build resolve --origin_instance <instance-id>`,
		Args: cobra.NoArgs,
	}
	...
	cmd.RunE = fncobra.RunE(func(ctx context.Context, _ []string) error {
		...
		if provided == 0 {
			return fnerrors.Newf("one of --github_job, --buildkite_job, or --origin_instance is required")
		}
		if provided > 1 {
			return fnerrors.Newf("origin flags (--github_job, --buildkite_job, and --origin_instance) are mutually exclusive")
		}
