...
export function ServerTabs(props: { server: StackEntryType; current: string }) {
	...
	let constructors: { [key: string]: (tabProps: TabProps) => JSX.Element } = {
		logs: (tabProps: TabProps) => <ServerTerminal key={`logs/${id}`} serverId={id} {...tabProps} />,
		terminal: (tabProps: TabProps) => (
			<ServerTerminal key={`terminal/${id}`} serverId={id} wireStdin={true} {...tabProps} />
		),
	};
	...
	return (
		<>
			<TerminalTabs
				...
				current={props.current}></TerminalTabs>
