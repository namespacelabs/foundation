...
func (p *ParsedVersions) SDK(version string, platform specs.Platform, makeURL func(string, specs.Platform) (string, string)) (compute.Computable[LocalSDK], error) {
	...
	if !has {
		return nil, fnerrors.Newf("%s/sdk: no configuration for version %q", p.Name, version)
	}

	arts, has := v.Artifacts[actualVer]
	if !has {
		return nil, fnerrors.Newf("%s/sdk: no configuration for version %q (was %q)", p.Name, actualVer, version)
	}

	key := fmt.Sprintf("%s/%s", platform.OS, platform.Architecture)
	digest, has := arts[key]
	if !has {
		return nil, fnerrors.Newf("%s/sdk: no platform configuration for %q in %q", p.Name, key, actualVer)
	}
