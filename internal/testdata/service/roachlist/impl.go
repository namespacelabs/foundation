...
func (svc *Service) Add(ctx context.Context, req *proto.AddRequest) (*emptypb.Empty, error) {
	...
	if err := add(ctx, svc.db, req.Item); err != nil {
		log.Printf("failed to add list item: %v", err)
		return nil, err
	}
...
func list(ctx context.Context, db *postgres.DB) ([]string, error) {
	...
	rows, err := db.Query(ctx, "SELECT Item, crdb_region FROM list;")
	if err != nil {
	...
	var res []string
	for rows.Next() {
		var item, region pgtype.Text
		err = rows.Scan(&item, &region)
		if err != nil {
			return nil, err
		}
		res = append(res, item.String)

		log.Printf("item: %s, region: %s\n", item, region)
	}
...
func (svc *Service) List(ctx context.Context, _ *emptypb.Empty) (*proto.ListResponse, error) {
	...
	if err != nil {
		log.Printf("failed to read list: %v", err)
		return nil, err
	}
