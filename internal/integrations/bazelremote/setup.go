...
func setup(ctx context.Context) (_ Config, retErr error) {
	...
	if response.GetSchedulerEndpoint() == "" || response.GetStorageEndpoint() == "" {
		return Config{}, fnerrors.Newf("received incomplete Bazel execution cluster response (scheduler=%q storage=%q)", response.GetSchedulerEndpoint(), response.GetStorageEndpoint())
	}
