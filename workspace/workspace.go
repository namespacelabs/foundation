...
// ModuleAt returns a schema.WorkspaceData with a reference to the workspace filename tried, even when errors are returned.
func ModuleAt(path string) (*schema.WorkspaceData, error) {
	data := &schema.WorkspaceData{AbsPath: path, DefinitionFile: workspaceFilename}
